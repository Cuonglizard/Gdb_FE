...
let onkeyup_save_filename = (e: any) => {
  if (e.keyCode === constants.ENTER_BUTTON_NUM) {
    const filename = e.currentTarget.value;
    if (filename.trim()) {
      // Lưu tên file vào store hoặc thực hiện hành động lưu
      store.set("current_filename", filename);
      console.log("Filename saved:", filename);
      // Có thể gọi hàm save với tên file mới
      if (window && typeof window.saveCurrentCode === "function") {
        window.saveCurrentCode();
      }
    }
  }
};
    ...
    return (
      ...
      >
        ...
        <div style={{ marginTop: 3, whitespace: "nowrap" }} className="flexrow">
          ...
          {/* imput file name  */}
          <input
          onKeyUp={onkeyup_save_filename}
          autoComplete="on"
          title="Enter filename, then press enter to save"
        ...
        <button
          ...
          className="btn btn-default btn-xs"
          title="Save current source code to server"
          onClick={() => {
            if (window && typeof window.saveCurrentCode === "function") {
              window.saveCurrentCode();
            } else {
              console.warn("saveCurrentCode function not available");
