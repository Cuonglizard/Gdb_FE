...
class SourceCode extends React.Component<{}, State> {
  ...
  window.saveCurrentCode = this.saveCodeToServer.bind(this);
    // bind methods
...
int main() {
    cout << "Hello CodeMirror in C++!" << endl;
    return 0;
}`,
    };
  }
  ...
  CommitCodeToServer( value: any) {
  //this.setState({ code: value });
    console.log(`Write line in SourceCode component: ${value}`);  
  }
  async sendCodeToServer() {
    try {
      
      // const response = await axios.post("/api/compile", { code: this.state.code });
  ...
  componentDidUpdate() {
    ...
    if (obj && obj.source_code_raw && this.state.code !== obj.source_code_raw) {
      this.setState({ code: obj.source_code_raw });
    }
...
saveCodeToServer = () => {
  ...
  const fileDomain = "/home/cuongtran/Gdbui_project/gdbgui_change/gdbgui/examples/cpp/";
  const currentFile = store.get("current_filename");
  if (!currentCode) {
  ...
  if (!currentFile) {
    alert("No file name! Please enter a filename before saving.");
    return;
  }
  const filePath = fileDomain + currentFile;


  // Backend expects JSON with "path" and "sourceCode" fields
  const data = {
    path: filePath,
    sourceCode: currentCode  // Đổi từ "source_code" thành "sourceCode"
  };
  ...
  $.ajax({
    beforeSend: function(xhr) {
      ...
      xhr.setRequestHeader("Content-Type", "application/json"); // Thêm Content-Type
    },
    url: "/save_source_code",
    cache: false,
    type: "POST",
    data: JSON.stringify(data), // Chuyển thành JSON string
    success: function(response) {
      if (response.message) {
        alert("Code saved successfully!");
        
        // Hiển thị kết quả compile nếu có
        if (response.compile_result) {
          if (response.compile_result.success) {
            Actions.add_console_entries(
              `File ${currentFile} saved and compiled successfully! Executable: ${response.compile_result.exec_file}`,
              constants.console_entry_type.GDBGUI_OUTPUT
            );
          } else {
            Actions.add_console_entries(
              `File saved but compilation failed: ${response.compile_result.stderr || response.compile_result.error}`,
              constants.console_entry_type.STD_ERR
            );
          }
        } else {
          Actions.add_console_entries(
            `File ${currentFile} saved successfully`,
            constants.console_entry_type.GDBGUI_OUTPUT
          );
        }
      } else {
        alert("Failed to save code: " + (response.message || "Unknown error"));
      }
    },
    error: function(response) {
      ...
      if (response.responseJSON && response.responseJSON.message) {
        ...
        errorMsg = `${response.statusText} (${response.status} error)`;
      }
      
      alert("Error saving code: " + errorMsg);
      Actions.add_console_entries(
        errorMsg,
        constants.console_entry_type.STD_ERR
      );
...
getCsrfToken = () => {
  // Lấy CSRF token từ Flask session thay vì meta tag
